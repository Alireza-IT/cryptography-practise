...
server.send(input)
encrypted_cookie = server.recv(1024)  # bytes
...
print("-------")
print(
    profile_for(input.decode())
)  # figure out what is happening and see the object generated from the user
print(
    encode_profile(profile_for(input.decode()))
)  # the string that we are expecting to do
prof = encode_profile(profile_for(input.decode()))
print(prof[0:16])
print(prof[16:32])  # second block
print(prof[32:48])  # <-- to substitute # 3rd block and can be omitted
...
new_input = b"aaaaaaaaaa" + admin_full_block  # admin starts from second block
...
final_cookie = encrypted_cookie[0:32] + encrypted_cookie2[16:32]  # to perform attack
